...
def _build_invoice_payload(order: sales_models.Order, items) -> Dict[str, Any]:
    ...
    return {
        ...
        "forma_pago": "01",
        "metodo_pago": "PUE",
        "uso_cfdi": order.bill_uso_cfdi or "G03",
        "subtotal": order.subtotal or 0,
        "total": order.total_amount or 0,
        "receptor": {
            "rfc": order.bill_rfc or "XAXX010101000",
            "nombre": order.bill_legal_name or (order.customer.name if order.customer else "Público general"),
            "uso_cfdi": order.bill_uso_cfdi or "G03",
            "regimen_fiscal": order.bill_regimen_fiscal or "616",
