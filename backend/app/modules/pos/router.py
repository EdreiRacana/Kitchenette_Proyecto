...
async def search_products(db: DB, current_user: CurrentUser,
                          ...
                          available_only: bool = Query(
                              False,
                              description="Si true, oculta productos con stock 0 "
                                          "en el almacen del terminal activo del cajero.",
                          )):
...
async def popular_products(db: DB, current_user: CurrentUser,
    ...
    """Top vendidos en los últimos 30 días EN EL ALMACEN DEL POS — para el grid
    de acceso rápido. Si no hay sesion abierta, cae al top global."""
